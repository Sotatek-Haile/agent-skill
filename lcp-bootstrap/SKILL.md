...
```
/lcp:bootstrap [requirements-file] [wbs-file] [--be|--fe|--fullstack] [--with-spec-kit] [--validate]
```
...
**Examples:**
```
/lcp:bootstrap                                     ← wizard mode (recommended)
...
/lcp:bootstrap --validate                          ← validate + selective update
```
...
Next steps:
  ...
  4. Run `/lcp:bootstrap --validate` periodically to keep context fresh
```
